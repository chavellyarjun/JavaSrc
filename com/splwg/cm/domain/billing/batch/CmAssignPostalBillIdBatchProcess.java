 ...
 * PROGRAM DESCRIPTION:
 *
 * This Batch Process is used to assign Postal Bill Id to Bill.
 ...
 *      softParameters = { @BatchJobSoftParameter (entityName = billRouteType, name = postalBillRouteType, required = true, type = entity)
 *            , @BatchJobSoftParameter (entityName = characteristicType, name = postalBillIdCharType, required = true, type = entity)})
 */
public class CmAssignPostalBillIdBatchProcess extends
		CmAssignPostalBillIdBatchProcess_Gen {
	
	 private static final com.splwg.shared.logging.Logger logger = com.splwg.shared.logging.LoggerFactory
	            .getLogger(CmAssignPostalBillIdBatchProcess.class);


	public JobWork getJobWork() {
		
		Query unassignedBillQry = createQuery("from AccountPerson ap, Bill bill where ap.id.account.id = bill.account.id and "
				+ "ap.billRouteTypeId = :billRouteTypeId and bill.billStatus = :billStatus and not exists ( select bc from BillCharacteristic bc where bc.id.bill.id = bill.id  and bc.id.characteristicType.id = :postalBillIdCharTypeId )","");
		unassignedBillQry.bindId("billRouteTypeId", getParameters().getPostalBillRouteType().getId());
		unassignedBillQry.bindLookup("billStatus", BillStatusLookup.constants.COMPLETE);
		unassignedBillQry.bindId("postalBillIdCharTypeId", getParameters().getPostalBillIdCharType().getId());
	...
	public Class<CmAssignPostalBillIdBatchProcessWorker> getThreadWorkerClass() {
		return CmAssignPostalBillIdBatchProcessWorker.class;
	}

	public static class CmAssignPostalBillIdBatchProcessWorker extends
			CmAssignPostalBillIdBatchProcessWorker_Gen {
		...
		public boolean executeWorkUnit(ThreadWorkUnit unit)
			...
			System.out.println(bill.getId().getTrimmedValue());
			try{
				String customBillId = CmAddPostalBillIdToBill.addPostalBillId(bill, getParameters().getPostalBillIdCharType());
				logMessage(CustomMessageRepository.postalBillIdAssignedtoBill(customBillId, bill.getId().getTrimmedValue()), MessageSeverityLookup.constants.INFORMATIONAL);
				return true;
			}catch(Exception e){
			    logger.error("", e);
				throw new RunAbortedException(CustomMessageRepository.assignPostalBillIdBatchAborted(getBatchControlId().getTrimmedValue(),getBatchNumber().toString(),bill.getId().getTrimmedValue() ,e.getMessage()));
			}
